...
if __name__ == "__main__":    
    ...
    l0_threshold = 0.02
    # e.g., lambdaG=0.1 for n=5 and 0.2 for n=8 (for ground truth scores)
    lambdaG = 0.1
    ...
    for nd_idx, (n, d) in enumerate(nd_list):
        ...
        for run_idx in range(nruns):
            ...
            if estimate_score_fns == True:
                # note that the algorithm requires not too many samples for learning the decoder (e.g. 300 in our experiments). However, score estimation requires far too many samples. Hence, we create samples only for score estimation part here.
                ...
                dsz_bw_hards = (torch.linalg.vector_norm(dszs_bw_hards, ord=1, dim=1) / nsamples).T
                dsz_obs_hard_1 = (torch.linalg.vector_norm(dszs_obs_hard_1, ord=1, dim=1) / nsamples).T
                dsz_obs_hard_2 = (torch.linalg.vector_norm(dszs_obs_hard_2, ord=1, dim=1) / nsamples).T
            ...
            try:
                u = g_scale_i_glm_tanh(d=d,n=n,nsamples=nsamples,dsxs_bw_hards=dsxs_bw_hards,xs_obs=xs_obs,xs_hard_1=xs_hard_1,xs_hard_2=xs_hard_2,learning_rate=learning_rate,nsteps_max=nsteps_max,lambda_recon=lambda_recon,l0_threshold=l0_threshold)
            ...
            shd =  utils.structural_hamming_distance(dag_gt,dag_hat_th)
            tp, fp, fn, tn = utils.cm_graph_entries(dag_gt,dag_hat_th)
            ...
            results_run["edge_precision"] = tp / (tp + fp)
            results_run["edge_recall"] = tp / (tp + fn)
